...
namespace MinimalAPIPeliculas.Endpoints
{
    public static class ComentariosEndpoints
    {
        public static RouteGroupBuilder MapComentarios(this RouteGroupBuilder group)
        {
            ...
            group.MapPut("/{id:int}", Actualizar).RequireAuthorization(); 
        ...
        static async Task<Results<Created<ComentarioDTO>, NotFound, BadRequest<string>>> Crear(int peliculaId, 
            ...
            IRepositorioPeliculas repositorioPeliculas, IMapper mapper, IOutputCacheStore outputCacheStore,IServicioUsuario servicioUsuario)
        {
            if (!await repositorioPeliculas.Existe(peliculaId))
            {
                return TypedResults.NotFound();
        ...
        static async Task<Results<NoContent, NotFound,BadRequest<string>,ForbidHttpResult >> Actualizar(int peliculaId, int id, 
            CrearComentarioDTO crearComentarioDTO, IOutputCacheStore outputCacheStore,
            IRepositorioComentarios repositorioComentarios, IRepositorioPeliculas repositorioPeliculas,
            IMapper mapper, IServicioUsuario servicioUsuario)
        {
            if (!await repositorioPeliculas.Existe(peliculaId))
            {
                return TypedResults.NotFound();
        ...
        static async Task<Results<NoContent, NotFound, BadRequest<string>, ForbidHttpResult>> Borrar(int peliculaId, int id,
            IRepositorioComentarios repositorio, IOutputCacheStore outputCacheStore, IServicioUsuario servicioUsuario)
        {
